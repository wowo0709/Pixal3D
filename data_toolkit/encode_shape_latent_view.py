...
if __name__ == '__main__':
        ...
        def loader(task):
            ...
            try:
                ...
                if num_tokens is not None:
                    src_scale_path = Path(opt.dual_grid_root) / f'dual_grid_view_{opt.resolution}' / sha256 / f'view{view_idx:02d}_scale.json'
                ...
                _put_with_timeout(
                    load_queue,
                    (sha256, view_idx, vertices, intersected),
                    opt.timeout_seconds,
                )
    ...
    _atomic_write_csv(
        records,
        Path(opt.shape_latent_root) / 'shape_latents' / latent_view_name / 'new_records' / f'part_{opt.rank}.csv',
    )
